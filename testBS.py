...
if __name__ == "__main__":
    ...
    # 存入字典
        results.append({
            ...
            'set_size_distribution': str(sizes_distribution.tolist()),
            'label_stratified_coverage': str(label_stratified_coverage),
            'label_distribution': str(label_distribution),
            'avg_avg_size': avg_avg_size
        })

# 保存到CSV
    df = pd.DataFrame(results)
    df.to_csv('testBS.csv', index=False)
